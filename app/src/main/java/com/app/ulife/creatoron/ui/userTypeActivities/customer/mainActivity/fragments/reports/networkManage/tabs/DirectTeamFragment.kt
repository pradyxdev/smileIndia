...
import com.app.ulife.creatoron.adapters.networkManage.DirectTeamAdapter
...
import com.app.ulife.creatoron.models.networkManage.directTeam.GetDirectTeamRes
import com.app.ulife.creatoron.models.networkManage.directTeam.Obj
...
class DirectTeamFragment : Fragment(), KodeinAware, DirectTeamAdapter.OnItemClickListener {
    ...
    private fun setupViews() {
        binding.apply {
            ...
            binding.tvToolbarTitle.text = "Direct Team Report"
            hitApis()
        }
    }
    ...
    private fun hitApis() {
        getReport(
            GetDirectTeamReq(
                apiname = "GetDirectUsers", obj = Obj(
                    from = "",
                    sp = "",
                    status = "",
                    to = "",
    ...
    private fun getReport(req: GetDirectTeamReq) {
        LoadingUtils.showDialog(context, isCancelable = false)
        viewModel.getDirectTeamHistory = MutableLiveData()
        viewModel.getDirectTeamHistory.observe(requireActivity()) {
            try {
                val response = Gson().fromJson(it, GetDirectTeamRes::class.java)
                if (response != null) {
                    if (response.status) {
                        ...
                        binding.apply {
                            ...
                            rvList.apply {
                                adapter = DirectTeamAdapter(
                                    context,
                                    response.data,
                                    this@DirectTeamFragment
                                )
        ...
        viewModel.getDirectTeamHistory(req)
    }
