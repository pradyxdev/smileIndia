...
package com.app.ulife.creatoron.ui.userTypeActivities.customer.mainActivity.fragments.reports.networkManage.tabs
...
import com.app.ulife.creatoron.adapters.networkManage.LevelWiseAdapter
...
import com.app.ulife.creatoron.models.networkManage.levelWise.CustomData
import com.app.ulife.creatoron.models.networkManage.levelWise.GetLevelWiseReq
import com.app.ulife.creatoron.models.networkManage.levelWise.GetLevelWiseRes
import com.app.ulife.creatoron.models.networkManage.levelWise.Obj
...
class LevelTeamFragment : Fragment(), KodeinAware, LevelWiseAdapter.OnItemClickListener {
    ...
    private fun setupViews() {
        binding.apply {
            ...
            binding.tvToolbarTitle.text = "Level Team Report"
            hitApis()
        }
    }
    ...
    private fun hitApis() {
        getReport(
            GetLevelWiseReq(
                apiname = "GetLevelWiseTeam", obj = Obj(
                    from = "",
                    status = "",
                    to = "",
    ...
    private fun getReport(req: GetLevelWiseReq) {
        LoadingUtils.showDialog(context, isCancelable = false)
        viewModel.getLevelWiseHistory = MutableLiveData()
        viewModel.getLevelWiseHistory.observe(requireActivity()) {
            try {
                val response = Gson().fromJson(it, GetLevelWiseRes::class.java)
                if (response != null) {
                    if (response.status) {
                            ...
                            for (i in response.data.indices) {
                                list.add(
                                    CustomData(
                                        ...
                                        deactive = "" + response.data[i].deactive,
                                        levelcount = "" + response.data[i].levelcount,
                                    )
                                )
                            }

                            rvList.apply {
                                adapter = LevelWiseAdapter(
                                    context,
                                    list,
                                    this@LevelTeamFragment
                                )
        ...
        viewModel.getLevelWiseHistory(req)
    }
