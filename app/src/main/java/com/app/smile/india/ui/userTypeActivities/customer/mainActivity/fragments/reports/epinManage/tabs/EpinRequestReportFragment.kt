...
package com.app.smile.india.ui.userTypeActivities.customer.mainActivity.fragments.reports.epinManage.tabs
...
import com.app.smile.india.adapters.epinManage.EpinReqReportAdapter
...
import com.app.smile.india.models.epinManage.epinReqReport.EpinReqReportRes
...
class EpinRequestReportFragment : Fragment(), KodeinAware,
    EpinReqReportAdapter.OnItemClickListener {
    ...
    private fun setupViews() {
        binding.apply {
            ...
            binding.tvToolbarTitle.text = "E-Pin Request Report"
            hitApis()
        }
    }
    ...
    private fun hitApis() {
        getEpinReqReport(
            EpinReqReportReq(
                apiname = "GetEPinRequestList",
                obj = com.app.smile.india.models.epinManage.epinReqReport.Obj(
                    Type = "",
                    from = "",
                    status = "",
                    to = "",
    ...
    private fun getEpinReqReport(req: EpinReqReportReq) {
        LoadingUtils.showDialog(context, isCancelable = false)
        viewModel.getEpinReqReport = MutableLiveData()
        viewModel.getEpinReqReport.observe(requireActivity()) {
            try {
                val response = Gson().fromJson(it, EpinReqReportRes::class.java)
                if (response != null) {
                    if (response.status) {
                        ...
                        binding.apply {
                            ...
                            rvList.apply {
                                adapter = EpinReqReportAdapter(
                                    context,
                                    response.data,
                                    this@EpinRequestReportFragment
                                )
        ...
        viewModel.getEpinReqReport(req)
    }
